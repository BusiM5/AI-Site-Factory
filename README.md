...
On Render, durable SQLite storage requires the paid Starter instance and the 1 GB persistent disk declared in `render.yaml`. The database path is `/var/data/pipeline.db` and uploaded campaign files use `/var/data/uploads`. As a recovery safety net, an empty Render database rebuilds only the managed Zendesk tickets that have entered the deploy/cancel workflow; set `ENABLE_ZENDESK_CAMPAIGN_RECOVERY_ON_STARTUP=false` to disable that behavior. This recovery is not a substitute for the persistent disk because untouched intake campaigns and import files do not have a complete external copy.
