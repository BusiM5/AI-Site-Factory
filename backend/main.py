...
EXISTING_GENERATED_SITE_REFRESH_LOCK = threading.Lock()
...
class ExistingGeneratedSiteRefreshRequest(BaseModel):
    zendeskTicketId: int = Field(gt=0)
    channel: str
    repository: str
    expectedCommitSha: str
...
def normalize_generated_html_for_compare(site_html: str) -> str:
    """Normalize transport-only whitespace without changing deployable source."""

    normalized = str(site_html or "").lstrip("\ufeff").replace("\r\n", "\n").replace("\r", "\n")
    return "\n".join(line.rstrip() for line in normalized.split("\n") if line.strip()).strip()


def validated_netlify_live_url(value: Any) -> Tuple[str, str, str]:
    live_url = compact_text(value)
    parsed = urlparse(live_url)
    try:
        parsed_port = parsed.port
    except ValueError as error:
        raise HTTPException(status_code=409, detail="The Zendesk live URL contains an invalid port.") from error
    hostname = compact_text(parsed.hostname).lower()
    valid_host = bool(
        re.fullmatch(r"[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.netlify\.app", hostname)
    )
    if (
        ...
        or compact_text(parsed.query)
        or compact_text(parsed.fragment)
        or compact_text(parsed.params)
        or parsed.path not in {"", "/"}
        or compact_text(parsed.netloc).lower() != hostname
    ):
        raise HTTPException(
            status_code=409,
            detail="The deployed ticket must contain one canonical HTTPS *.netlify.app root URL.",
        )
    site_slug = hostname[: -len(".netlify.app")]
    return f"https://{hostname}", hostname, site_slug


def github_text_file_at_commit(
    repository: str,
    path: str,
    commit_sha: str,
    headers: Dict[str, str],
    ...
    if (
        ...
        or compact_text(payload.get("encoding")).lower() != "base64"
        or not isinstance(payload.get("content"), str)
        or int(payload.get("size") or 0) > max_bytes
    ):
        raise HTTPException(status_code=409, detail=f"GitHub {path} is not a supported regular text file.")
    encoded = re.sub(r"\s+", "", payload["content"])
    try:
        content = base64.b64decode(encoded, validate=True)
    except (ValueError, binascii.Error) as error:
        raise HTTPException(status_code=409, detail=f"GitHub {path} has invalid base64 content.") from error
    if len(content) > max_bytes:
        raise HTTPException(status_code=409, detail=f"GitHub {path} exceeds the refresh size limit.")
    try:
        return content.decode("utf-8")
    except UnicodeDecodeError as error:
        raise HTTPException(status_code=409, detail=f"GitHub {path} must be UTF-8 text.") from error


def live_zendesk_form_field_ids(channel: str, contract: Dict[str, Any]) -> Dict[str, str]:
    """Resolve the unique active blueprint fields on the configured live form."""

    form_id = compact_text(contract.get("formId"))
    brand_id = compact_text(contract.get("brandId"))
    form = zendesk_api_request("get", f"/ticket_forms/{form_id}.json").get("ticket_form") or {}
    form_field_ids = {compact_text(value) for value in (form.get("ticket_field_ids") or [])}
    restricted_brands = {compact_text(value) for value in (form.get("restricted_brand_ids") or [])}
    if (
        compact_text(form.get("id")) != form_id
        or not form.get("active", True)
        or (not form.get("in_all_brands") and brand_id not in restricted_brands)
    ):
        raise HTTPException(status_code=409, detail="The configured Zendesk channel form is no longer authoritative.")

    live_fields = zendesk_list_all("/ticket_fields.json", "ticket_fields")
    resolved: Dict[str, str] = {}
    for definition in ZENDESK_FIELD_BLUEPRINT:
        if channel not in definition["forms"]:
            continue
        matches = [
            item
            for item in live_fields
            if compact_text(item.get("id")) in form_field_ids
            and compact_text(item.get("title")) == definition["title"]
            and item.get("active", True)
        ]
        if len(matches) != 1:
            raise HTTPException(
                status_code=409,
                detail=f"The live Zendesk form does not have one unique {definition['title']} field.",
            )
        resolved[definition["key"]] = compact_text(matches[0].get("id"))
    return resolved


def generated_site_refresh_result(
    *,
    action: str,
    request: ExistingGeneratedSiteRefreshRequest,
    ticket_values: Dict[str, str],
    live_url: str,
    site_id: str,
    source_html: str,
    live_html: str,
    ...
    return {
        ...
        "campaignId": ticket_values["campaignId"],
        "canonicalLeadKey": ticket_values["canonicalLeadKey"],
        "approvalId": ticket_values["approvalId"],
        "businessName": ticket_values["businessName"],
        "repository": request.repository,
        "commitSha": request.expectedCommitSha.lower(),
        "liveUrl": live_url,
        "siteId": site_id,
        "deployId": deploy_id,
        "sourceChecksum": html_checksum(source_html),
        "sourceNormalizedChecksum": html_checksum(normalize_generated_html_for_compare(source_html)),
        "liveChecksum": html_checksum(live_html),
        "liveNormalizedChecksum": html_checksum(normalize_generated_html_for_compare(live_html)),
    }


@app.post("/api/admin/existing-generated-site/refresh", include_in_schema=False)
def refresh_existing_generated_site(
    request: ExistingGeneratedSiteRefreshRequest,
    http_request: Request,
):
    """Temporarily refresh one verified existing site without creating or regenerating anything."""
    ...
    if not EXISTING_GENERATED_SITE_REFRESH_LOCK.acquire(blocking=False):
        raise HTTPException(status_code=409, detail="An existing-site refresh is already running.")

    try:
        channel = compact_text(request.channel).lower()
        contract = require_zendesk_ticket_contract(channel)
        ticket = zendesk_api_request("get", f"/tickets/{request.zendeskTicketId}.json").get("ticket") or {}
        ticket_fields = {
            compact_text(item.get("id")): item.get("value")
            for item in (ticket.get("custom_fields") or [])
        }
        field_ids = dict(contract["fieldIds"])

        def ticket_field(key: str) -> Any:
            return ticket_fields.get(compact_text(field_ids.get(key)))

        if compact_text(ticket.get("id")) != str(request.zendeskTicketId):
            raise HTTPException(status_code=409, detail="Zendesk returned a different ticket.")
        if (
            compact_text(ticket.get("brand_id")) != compact_text(contract["brandId"])
            or compact_text(ticket.get("ticket_form_id")) != compact_text(contract["formId"])
        ):
            raise HTTPException(status_code=409, detail="The ticket no longer uses the configured brand and channel form.")
        if compact_text(ticket.get("status")).lower() in {"solved", "closed"}:
            raise HTTPException(status_code=409, detail="Solved or closed tickets cannot be refreshed.")
        ...
        if (
            any(not compact_text(ticket_field(key)) for key in configured_identity_keys)
            or not zendesk_ticket_field_value_matches(True, ticket_field("deployRequested"))
        ):
            field_ids = live_zendesk_form_field_ids(channel, contract)

        ticket_values = {
            key: compact_text(ticket_field(key))
            for key in (
                "campaignId",
                "pipelineId",
                "canonicalLeadKey",
        ...
        missing_values = [key for key, value in ticket_values.items() if not value]
        if missing_values:
            raise HTTPException(
                status_code=409,
                detail=f"The deployed ticket is missing managed values: {', '.join(missing_values)}.",
            )
        if ticket_values["pipelineId"] != ticket_values["campaignId"]:
            raise HTTPException(status_code=409, detail="The ticket campaign and pipeline identities do not match.")
        expected_external_id = (
            f"asf:{ticket_values['campaignId']}:{ticket_values['canonicalLeadKey']}:{channel}:intake"
        )
        if compact_text(ticket.get("external_id")) != expected_external_id:
            raise HTTPException(status_code=409, detail="The ticket external identity is not authoritative.")
        if not zendesk_ticket_field_value_matches(True, ticket_field("deployRequested")):
            raise HTTPException(status_code=409, detail="The deployed ticket is not approved for deployment.")
        if compact_text(ticket_field("leadStatus")) != "asf_cf_status_deployed":
            raise HTTPException(status_code=409, detail="The ticket is not in the deployed lead state.")
        if compact_text(ticket_field("contactChannel")) != f"asf_cf_channel_{channel}":
            raise HTTPException(status_code=409, detail="The ticket contact channel does not match the requested route.")

        tags = {compact_text(tag).lower() for tag in (ticket.get("tags") or []) if compact_text(tag)}
        ...
        required_tags = {
            ...
            f"asf_channel_{channel}",
            form_tag,
            f"asf_deploy_{channel}_fired",
        }
        missing_tags = sorted(required_tags - tags)
        denied_tags = sorted(
            tag
            for tag in tags
            if tag in {"asf_duplicate_retired", "asf_do_not_deploy"}
            or tag.startswith("asf_superseded_by_")
        )
        if missing_tags or denied_tags:
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "SITE_REFRESH_TICKET_NOT_AUTHORITATIVE",
                    "missingTags": missing_tags,
                    "deniedTags": denied_tags,
                },
            )

        live_url, live_hostname, site_slug = validated_netlify_live_url(ticket_values["liveUrl"])
        expected_sha = compact_text(request.expectedCommitSha).lower()
        if not re.fullmatch(r"[0-9a-f]{40}", expected_sha):
            raise HTTPException(status_code=400, detail="expectedCommitSha must be one full Git commit SHA.")
        configured_owner = require_env("GITHUB_OWNER")
        repository_parts = compact_text(request.repository).split("/")
        if (
            len(repository_parts) != 2
            or not all(re.fullmatch(r"[A-Za-z0-9_.-]+", part or "") for part in repository_parts)
            or repository_parts[0].lower() != configured_owner.lower()
        ):
            raise HTTPException(status_code=409, detail="The repository is outside the configured GitHub owner.")
        ...
        repository_data = repository_response.json() or {}
        if (
            compact_text(repository_data.get("full_name")).lower() != request.repository.lower()
            or compact_text((repository_data.get("owner") or {}).get("login")).lower() != configured_owner.lower()
            or compact_text(repository_data.get("default_branch")) != "main"
            or bool(repository_data.get("archived"))
            or bool(repository_data.get("disabled"))
        ):
            raise HTTPException(status_code=409, detail="The GitHub repository identity or state is not eligible for refresh.")
        commit_response = requests.get(
            f"https://api.github.com/repos/{request.repository}/commits/{expected_sha}",
            headers=gh_headers,
            timeout=30,
        )
        commit_response.raise_for_status()
        if compact_text((commit_response.json() or {}).get("sha")).lower() != expected_sha:
            raise HTTPException(status_code=409, detail="GitHub did not return the exact requested commit.")

        source_html = github_text_file_at_commit(
            request.repository, "index.html", expected_sha, gh_headers
        )
        readme = github_text_file_at_commit(
            request.repository, "README.md", expected_sha, gh_headers, max_bytes=200_000
        )
        canonical_match = re.search(r"Canonical lead key:\s*`([^`]+)`", readme, flags=re.IGNORECASE)
        checksum_match = re.search(r"HTML checksum:\s*`([0-9a-f]{64})`", readme, flags=re.IGNORECASE)
        if (
            not canonical_match
            or compact_text(canonical_match.group(1)) != ticket_values["canonicalLeadKey"]
            or not checksum_match
            or checksum_match.group(1).lower() != html_checksum(source_html)
        ):
            raise HTTPException(status_code=409, detail="The GitHub README does not authenticate the ticket and HTML source.")
        if ensure_required_site_features(source_html) != source_html:
            raise HTTPException(status_code=409, detail="The committed HTML does not contain the current required site features.")

        netlify_headers = {
            "Authorization": f"Bearer {require_env('NETLIFY_AUTH_TOKEN')}",
            "User-Agent": "AI-Site-Factory",
        }
        site_response = requests.get(
            f"https://api.netlify.com/api/v1/sites/{live_hostname}",
            headers=netlify_headers,
            timeout=30,
        )
        if site_response.status_code in {403, 404}:
            raise HTTPException(
                status_code=409,
                detail="The existing Netlify site is unavailable to the configured production account; no replacement was created.",
            )
        site_response.raise_for_status()
        site = site_response.json() or {}
        site_id = compact_text(site.get("id"))
        canonical_site_hosts = {
            compact_text(urlparse(value).hostname).lower()
            for value in (site.get("ssl_url"), site.get("url"))
            if compact_text(value)
        }
        if (
            not site_id
            or (compact_text(site.get("name")) and compact_text(site.get("name")).lower() != site_slug)
            or live_hostname not in canonical_site_hosts
        ):
            raise HTTPException(status_code=409, detail="Netlify returned a different site identity.")
        linked_repositories: Set[str] = set()
        for candidate in (site.get("repo"), site.get("build_settings")):
            if not isinstance(candidate, dict):
                continue
            linked_repository = compact_text(candidate.get("repo_path") or candidate.get("repo"))
            if linked_repository:
                linked_repositories.add(linked_repository.lower())
        if linked_repositories and request.repository.lower() not in linked_repositories:
            raise HTTPException(status_code=409, detail="The Netlify site is linked to a different repository.")

        def fetch_live_html() -> str:
            live_response = requests.get(
                f"{live_url}/?asf_refresh={time.time_ns()}",
                headers={"Cache-Control": "no-cache", "User-Agent": "AI-Site-Factory"},
                timeout=30,
            )
            live_response.raise_for_status()
            if compact_text(urlparse(live_response.url).hostname).lower() != live_hostname:
                raise HTTPException(status_code=409, detail="The live site redirected to a different hostname.")
            return live_response.text

        current_live_html = fetch_live_html()
        normalized_source = normalize_generated_html_for_compare(source_html)
        if normalize_generated_html_for_compare(current_live_html) == normalized_source:
            return generated_site_refresh_result(
                ...
                live_html=current_live_html,
            )

        deploy_response = requests.post(
            f"https://api.netlify.com/api/v1/sites/{site_id}/deploys",
            headers={**netlify_headers, "Content-Type": "application/zip"},
            data=zip_site_html(source_html),
            timeout=90,
        )
        deploy_response.raise_for_status()
        deploy = deploy_response.json() or {}
        deploy_id = compact_text(deploy.get("id"))
        if not deploy_id or (
            compact_text(deploy.get("site_id"))
            and compact_text(deploy.get("site_id")) != site_id
        ):
            raise HTTPException(status_code=502, detail="Netlify returned a deploy for a different or missing site.")
        deploy_hosts = {
            compact_text(urlparse(value).hostname).lower()
            for value in (deploy.get("ssl_url"), deploy.get("url"))
            if compact_text(value)
        }
        if deploy_hosts and live_hostname not in deploy_hosts:
            raise HTTPException(status_code=502, detail="Netlify returned a deploy URL for a different site.")

        state = compact_text(deploy.get("state"), "uploaded").lower()
        poll_deadline = time.time() + max(15, min(int(os.getenv("NETLIFY_DEPLOY_POLL_SECONDS", "45")), 90))
        while state not in {"ready", "error"} and time.time() < poll_deadline:
            time.sleep(2)
            deploy_poll = requests.get(
                f"https://api.netlify.com/api/v1/deploys/{deploy_id}",
                headers=netlify_headers,
            ...
            deploy_poll.raise_for_status()
            deploy = deploy_poll.json() or {}
            if (
                compact_text(deploy.get("site_id"))
                and compact_text(deploy.get("site_id")) != site_id
            ):
                raise HTTPException(status_code=502, detail="Netlify deploy polling changed site identity.")
            state = compact_text(deploy.get("state"), state).lower()
        if state == "error":
            raise HTTPException(status_code=502, detail="The existing Netlify site refresh failed.")
        if state != "ready":
            raise HTTPException(status_code=504, detail="The existing Netlify site refresh did not become ready in time.")

        final_live_html = ""
        for attempt in range(12):
            final_live_html = fetch_live_html()
            if normalize_generated_html_for_compare(final_live_html) == normalized_source:
                break
            if attempt < 11:
                time.sleep(2)
        else:
            raise HTTPException(status_code=502, detail="Netlify reported ready but the canonical live HTML did not match GitHub.")

        return generated_site_refresh_result(
            action="REFRESHED_EXISTING_SITE",
            request=request,
            ticket_values=ticket_values,
            live_url=live_url,
            site_id=site_id,
            source_html=source_html,
            live_html=final_live_html,
            deploy_id=deploy_id,
        )
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Existing-site provider verification failed: {sanitize_message(error)}",
        ) from error
    finally:
        EXISTING_GENERATED_SITE_REFRESH_LOCK.release()
